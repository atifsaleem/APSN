...
fb_calendar = {
	...
	makeCalendar:function(year, month, day) {
		...
		this.wrt("<table>");
		this.wrt("<tr><th><a href='javascript:fb_calendar.makeCalendar("+(previous_month_year)+","+(previous_month)+");' title='"+this.month_names[previous_month]+" "+(previous_month_year)+"'>&lt;</a></th>");
		this.wrt("<th colspan='5' class='calendar-title'><select name='calendar-month' class='calendar-month' onChange='fb_calendar.makeCalendar("+year+",this.value);'>");
		...
		this.wrt("</select>");
		this.wrt("<select name='calendar-year' class='calendar-year' onChange='fb_calendar.makeCalendar(this.value, "+month+");'>");
		...
		this.wrt("</select></th>");
		this.wrt("<th><a href='javascript:fb_calendar.makeCalendar("+(next_month_year)+","+(next_month)+");' title='"+this.month_names[next_month]+" "+(next_month_year)+"'>&gt;</a></th></tr>");
		...
		for(var i=0;i<=5;i++) {
			...
			for(var j=0;j<7;j++) {
				...
				if(flag) {
					...
					this.wrt("<td class='days"+class_name+"'><a href='javascript:fb_calendar.selectDate(\""+year+"\",\""+mon+"\",\""+w+"\")'>"+w+"</a></td>");
					d++;
				} else {
		...
		this.wrt("</table>");
		this.wrt("<input type='button' value='Cancel' class='calendar-cancel' onclick='fb_calendar.hideCalendar();' />");
